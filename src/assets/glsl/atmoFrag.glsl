...
void main() {
	...
	float uScale = 1.5;
	float intensity = pow(0.2 - dot(vNormal, vec3(0, 0, 1.0)), 1.0);

    vec3 colorBeforeAngle = vec3(0.3, 0.6, 1.0) * intensity;
	...
	vec3 finalColor = mix(colorBeforeAngle, colorBeforeAngle, mixAmount);
